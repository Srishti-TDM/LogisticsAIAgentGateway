...
class LangChainService:
    ...
    async def run(self, message: str, domain: str = "risk") -> ChatResponse:
    ...
    async def _call_operations_agent(self, message: str) -> dict:
        """Call the inventory check-availability agent."""
        part_numbers = _extract_part_numbers(message)
        if not part_numbers:
            return {"error": "No part numbers found in your message. Please include at least one part number."}
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                resp = await client.post(
